...
pub unsafe fn inb(port: u16) -> u8 {
    ...
    asm!("in al, dx", out("al") ret, in("dx") port, options(nostack));
    ret
}
...
pub unsafe fn inw(port: u16) -> u16 {
    ...
    asm!("in ax, dx", out("ax") ret, in("dx") port, options(nostack));
    ret
}
...
pub unsafe fn inl(port: u16) -> u32 {
    ...
    asm!("in eax, dx", out("eax") ret, in("dx") port, options(nostack));
    ret
}
