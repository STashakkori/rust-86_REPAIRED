...
pub unsafe fn read() -> EFlags {
    let r: u32;
    asm!("pushfl; pop {}", out(reg) r, options(nostack));
...
pub unsafe fn set(val: EFlags) {
    asm!("push {}; popfl", in(reg) val.bits(), options(nostack, nomem));
}
...
#[inline(always)]
pub unsafe fn clac() {
    asm!("clac", options(nostack, nomem));
}
...
#[inline(always)]
pub unsafe fn stac() {
    asm!("stac", options(nostack, nomem));
}