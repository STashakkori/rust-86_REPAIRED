...
pub unsafe fn load_cs(sel: SegmentSelector) {
    ...
    asm!(
        "leaq ({}), {0}",
        "push {0}",
        "ret", // Adjusted from 'lretq' to 'ret' for demonstration, revisit this based on actual need
        sym load_cs_end, // This assumes you have a label 'load_cs_end' defined at the end of this function or block
        out(reg) tmp,
        in("rax") sel.bits() as usize,
        options(nostack)
    );
...
pub unsafe fn load_cs(sel: SegmentSelector) {
    asm!(
        "mov {0}, rax", // Correctly formatted assembly instruction
        in(reg) sel.bits() as u64,
        options(nostack)
    );
}
...
pub unsafe fn wrgsbase(base: u64) {
    asm!("wrgsbase {0}", in(reg) base, options(nostack));
}
...
pub unsafe fn wrfsbase(base: u64) {
    asm!("wrfsbase {0}", in(reg) base, options(nostack));
}
...
pub unsafe fn rdgsbase() -> u64 {
    let gs_base: u64;
    asm!("rdgsbase {}", out(reg) gs_base, options(nostack));
    gs_base
}
...
pub unsafe fn rdfsbase() -> u64 {
    let fs_base: u64;
    asm!("rdfsbase {}", out(reg) fs_base, options(nostack));
    fs_base
}
...
pub unsafe fn fs_deref() -> u64 {
    let fs: u64;
    asm!("mov {}, fs:0x0", out(reg) fs, options(nostack));
    fs
}
...
pub unsafe fn gs_deref() -> u64 {
    let gs: u64;
    asm!("mov {}, gs:0x0", out(reg) gs, options(nostack));
    gs
}
